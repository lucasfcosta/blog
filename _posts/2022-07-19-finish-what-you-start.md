...
Then, I'll uncover the reasons why some teams choose to work on multiple tasks concurrently despite that approach being suboptimal most of the time. In this section, I'll also explain when to batch tasks, when not to, and what you should do instead.
...
At the end, I have included a small summary for you to paste on Slack or Microsoft Teams and **convince your team to stop starting and start finishing**.
...
When _you_ arrive at this burger shop, the cook's job gets a bit more difficult. Now, they have to choose between preparing both our burgers concurrently, or one at a time (serially).
...
In this case, assuming I ordered only a bit before you did, my burger still takes four minutes. Yours, however, takes eight.
...
To make matters worse, let me show what happens if a friend of yours comes in, places an order, and the cooks decides to take the same shortsighted approach to burger-making.
...
In this scenario, I took ten minutes to taste my burger, and you took twelve. Thanks to your friend, I had to wait an extra six minutes, and you had to wait an extra three.

Such a disappointing burger-preparing performance demands vigorous action to be taken.

Personally, I'd prefer to educate the cook rather than kicking you and your friend out of the shop because I believe every hard-working engineer deserves a succulent burger on a Friday night.

Here's what we'll do. First, we'll illustrate what would've happened had the cook decided to prepare each of our burgers in series. Then, we'll enter the kitchen, hand him the schematics, and explain him what the optimal approach is.
...
As our drawing shows, had the cook prepared our three burgers in series, you and I would have had to wait the same amount of time for our burgers as before — 4 and 8 minutes. Your friend's burger would still have taken twelve minutes though, but that's what they deserve for being late for dinner.
...
Furthermore, working on these features concurrently demands B's specification to be written earlier than they would have been otherwise.
...
On the other hand, if you finish A before starting B, you'll deliver A earlier, and you'll buy yourself more time to work on B's specification. During that time, you can collect more information to boost B's chance of success.
...
As you can see, **in the software industry, whenever you start a new feature before finishing the first, the first will take longer to deliver.**
...
That's what just-in-time means: **instead of starting each feature as early as possible, you start them as late as you can afford to**, and incorporate as much feedback and information into the spec in the meantime.
...
As I've explained using the burger shop example, and as Little's Law dictates, when work in progress increases and throughput remains the same, cycle-times elongate. It's a straightforward mathematical fact. Nonetheless, many managers are unaware of it.
...
When preparing burgers, it's effortless to shift from working on one burger to another. When writing software, on the other hand, context-switching incurs a significant cost.
...
You and I delve into the kitchen with our drawing in hands, and the power of mathematics under our arms, and tell the cook:
...
> No matter how rare you like your patties, they all take a significant amount of time to prepare.<br>
> It's quicker for me to prepare them in larger batches than it is to wait for one patty to be ready before I toss another in the grill.

The cook has a point. Neither you or I know anything about the craft of burger-making, how could we expect to be right?

Nevertheless, there's a lesson to be taught here. One about batch-sizes and transaction costs.

Whenever it's expensive to move a work product, in this case a patty, from one stage of the production process to the next, that transition will happen less often, so that you can pay the transaction cost only once, but move multiple items at a time.
...
This is the exact reason why you don't go to the supermarket and buy one egg at a time. Going to the supermarket has a significant transaction cost, so you buy eggs in larger batches. In fact, the farther from your home the supermarket is, the more likely it is for the batch of eggs you bring home to be larger.

It's only when the cost of keeping the patties in the grill exceeds the cost of sending them to the counter that the cook will send then to the kitchen counter.

To summarise: whenever a transaction's cost goes up, that transaction happens less frequently. Furthermore, if the cost of holding items at a particular stage of the process is lower than the cost of pushing those items forward, it's better to hold them.

That's the also the reason because of which [Martin Fowler and Jez Humble](https://martinfowler.com/bliki/FrequencyReducesDifficulty.html) advocates that ["if something is painful, you should do it more often"](https://www.goodreads.com/quotes/1314241-if-it-hurts-do-it-more-frequently-and-bring-the), especially when it comes to deploying software. If you're forced to do something often, you'll naturally gravitate towards decreasing the cost of doing it.
...
In manufacturing, the cost of holding pieces in a factory is linear. The holding cost for a piece is just the cost of keeping it in storage multiplied by the number of days it's going to be there. On the other hand, in software development, the longer you hold a particular piece of work you have a compounding negative effect due to the lack of feeback, the larger deltas between versions, and the overall harm to the overall system reliability.
...
**Reducing transaction costs enables small batches. Small batches, in turn, reduce average cycle times, diminish risk, and increase enhance reliability**.

**You should only start without having finished whe transaction costs are high, and it wouldn't make economical sense to spend time decreasing them, either because you have agreed to a particular delivery, or because you don't have the capital to invest.**

I would be careful, however, to avoid falling into a situation in which "you're too busy draining the flood to be able to fix the leak". The earlier you decrease transaction costs, the earlier you'll be reaping the benefits from having done it.
...
In the burger shop's kitchen, there's a limit to how many patties you can put on the grill. Furthermore, at the end of the day, there are zero burgers in progress. "These constraints allow me to provide burger-eaters with predictable delivery times" — says the cook.
...
As the diagram shows, a customer which arrives early waits for a long time for their burger, while a customer which arrives later waits much less. In other words, there's a significant amount of variability in the time customers have to wait until they get their burger — shown by the area in blue.
...
This time, a customer which arrived early doesn't have to wait much longer than the customer which arrived later. As shown by the area in blue, there's less variability in the time it takes for customers to get burgers.
...
Additionally, if batch sizes vary, not only the waiting time within a particular batch of customers will be more variable, but also different batches of customers will see different variations in waiting time. Customers which arrive earlier may see cycle times from 1 to 10 minutes, while customers which arrive later might see their burgers taking anywhere from 1 to 30 minutes, for example.
...
That's why the cook mentioned it's helpful to have a limit on how many burgers they can fit on a grill: it makes batch sizes more uniform.

The same principle is valid for software development. The larger your batch sizes — that is, the more tasks you work on in parallel — the more varability in cycle-times you will see.
...
The lower you make WIP limits, and the more you stick to them, the less variability you'll see in cycle times.

Besides making your cycle-times unpredictable, the lack of WIP limits will also impact your ability to do forecasts.

Imagine, for example, that your team tends to work on many tasks concurrently. In that case, most of the days they'll be splitting their attention between tasks. Then, there will be a day in which they'll complete all those tasks, doing a large batch transferral.
...
In this histogram you can see there are a significant number of possible outcomes for this team: they may deliver anywhere from 0 to 90 tasks. Furthermore, there are plenty of likely scenarios in this distribution.

Now, let's change the throughput samples we used in our simulation. **This time, we'll consider the team works on one task at a time, delivering in more regular intervals**, instead of working on plenty of tasks in parallel and delivering them all in the same day.
...
Please notice that the team still delivered the same amount of tasks in that 10-day period. The only difference is that they delivered in a more uniform fashion.
...
If you compare these two histograms you'll see that the second histogram's distribution has fewer possible outcomes — 20 to 41 tasks instead of 0 to 90 —, and results are more clustered towards the center. In other words, its standard deviation is smaller.
...
The comparison between those three histogram leads us to conclude that teams which deliver tasks more uniformly are more predictable, even if they deliver the same number of tasks within the period we're using as the sample for our simulations.
...
Many times, the reason why teams will have multiple tasks in progress, is because moving a task from one stage to the next is costly. Consequently, these teams will attempt to transfer tasks from one stage to another in large batches, so that they pay this high transaction cost only once.

The problem with large batch transferrals in the software industry is that the cost of holding these tasks increases exponentially because their specifications perish, and change deltas increase, making it more difficult to find bugs a maintain the system's reliability on the face of more difficult change management.
